...
func TestBucketResetsAtWindowBoundary(t *testing.T) {
	b, advance := newBucketWithFakeClock(1, time.Minute)
	...
	if b.Allow() {
		t.Fatal("expected second request to be blocked before window boundary")
	}

	advance(time.Minute - time.Nanosecond)
	if b.Allow() {
		t.Fatal("expected request to remain blocked before window boundary")
	}

	advance(time.Nanosecond)
	if !b.Allow() {
		t.Fatal("expected request to be allowed at window boundary")
	}
}

func TestBucketSkipsMultipleIdleWindows(t *testing.T) {
	b, advance := newBucketWithFakeClock(2, time.Second)
	...
	for i := range 2 {
		if !b.Allow() {
			t.Fatalf("expected request %d to be allowed after idle windows", i+1)
		}
	}

	if b.Allow() {
		t.Fatal("expected request to be blocked after new window limit is reached")
	}
}
